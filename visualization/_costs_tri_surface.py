...
def create_multiple_costs_tri_surface(df: pd.DataFrame, save_fig: bool = False):
    ...
    fig, axs = plt.subplots(len(extractants), len(extractant_concentrations), figsize=(12, 10), subplot_kw = { 'projection': '3d' })
...
def costs_tri_surface(df_slice: pd.DataFrame, ax: plt.Axes):
    ...
    ax.set_xlabel('Nº Células', fontsize = 13)
    ax.set_ylabel('Razão A/O', fontsize = 13)
    ax.set_zlabel('Custo Total (USD/Kg)', labelpad = 14, fontsize = 13)

    ax.set_xticks(np.arange(3, 16, 3))
    ax.set_yticks(np.arange(0.5, 2.1, 0.5))

    if df_slice_copy['extractant'].str.contains('P507').any():
      ax.set_zticks(np.arange(50, 71, 6))

    if df_slice_copy['extractant'].str.contains('D2EHPA').any():
      ax.set_zticks(np.arange(50, 101, 15))

    if df_slice_copy['extractant'].str.contains('Cyanex').any():
      ax.set_zticks(np.arange(50, 66, 4))
...
def predict_values(df: pd.DataFrame) -> pd.DataFrame:
    n_cells, ao_ratio, pHi = np.meshgrid(np.linspace(2, 15, 37), np.linspace(0.5, 2, 100), np.linspace(1, 2, 11))
